...
{
	int a, b, n, i;
	struct polynomial f, g, A, B, AA, BB, X, Y, Z, W;
	f.leading = NULL;
	g.leading = NULL;
	g.degree = 0;
	A.leading = NULL;
			...
			if (is_OK(AA)) {
				...
				while (b < d) {
					...
					while (next_pol(B)) {
						...
						clean_pol(&BB);
						if ((is_AB(AA, BB)) && \
						(setup(X, AA, BB, f, 1, 1) > 0) && \
/*						(setup(Y, AA, BB, f, 1, 1) > 0) && \
						(setup(Z, AA, BB, f, 1, 1) > 0) && \
						(setup(W, AA, BB, f, 1, 1) > 0) && \ */
						(setup(AA, BB, f, g, 2, 1) > 1) && \
						(setup(AA, BB, g, g, 0, 1) == 2)) {
